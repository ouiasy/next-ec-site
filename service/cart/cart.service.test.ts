import {createCartRepository} from "@/repository/cart/cart.repository";
...
describe("cart service", () => {
  ...
  test("在庫切れの時の挙動", async () => {
    ...
    const cartRepoStub = createMockCartRepo(cart)
    const productRepoStub = createMockProductRepo([product1, product2, productOutofstock])
    ...
    const remainItem = cartDetail.items.find(item => item.productId === product1.id)
